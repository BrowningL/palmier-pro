...
struct SocialAudioActivityRange: Codable, Sendable, Equatable {
    ...
    private enum CodingKeys: String, CodingKey { case startSeconds, endSeconds }
    ...
    init(from decoder: Decoder) throws {
        ...
        self.init(
            ...
            endSeconds: (try? c.decode(Double.self, forKey: .endSeconds)) ?? 0
        )
    }
}
...
struct SocialAudioSettings: Codable, Sendable, Equatable {
    ...
    private enum CodingKeys: String, CodingKey {
        case role, preset, measuredLoudnessLUFS, measuredPeakDbFS
        case normalizationGainDb, speechActivity, duckingAmountDb
    }
    ...
    init(from decoder: Decoder) throws {
        let c = try decoder.container(keyedBy: CodingKeys.self)
        self.init(
            role: (try? c.decode(SocialAudioRole.self, forKey: .role)) ?? .voice,
            preset: (try? c.decode(SocialAudioPreset.self, forKey: .preset)) ?? .balanced,
            measuredLoudnessLUFS: try? c.decode(Double.self, forKey: .measuredLoudnessLUFS),
            measuredPeakDbFS: try? c.decode(Double.self, forKey: .measuredPeakDbFS),
            normalizationGainDb: (try? c.decode(Double.self, forKey: .normalizationGainDb)) ?? 0,
            speechActivity: (try? c.decode([SocialAudioActivityRange].self, forKey: .speechActivity)) ?? [],
            duckingAmountDb: (try? c.decode(Double.self, forKey: .duckingAmountDb)) ?? 0
        )
    }
