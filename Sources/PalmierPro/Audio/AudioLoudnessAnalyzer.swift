...
enum AudioLoudnessAnalyzerError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .invalidBlendStrength:
            "The dry/wet analysis strength must be between zero and one."
        case .unexpectedPCMFormat:
...
enum AudioLoudnessAnalyzer {
    ...
    static func analyze(
        ...
        return finish(
            ...
            range: range,
            firstPresentationSeconds: timing.firstPresentationSeconds
        )
    }

    /// Measures the same aligned dry/wet signal used by CompositionBuilder.
    static func analyzeBlend(
        dryURL: URL,
        wetURL: URL,
        strength: Double,
        range: ClosedRange<Double>? = nil
        ...
        guard strength.isFinite, (0...1).contains(strength) else {
            throw AudioLoudnessAnalyzerError.invalidBlendStrength
        }
        if strength == 0 { return try await analyze(from: dryURL, range: range) }
        if strength == 1 { return try await analyze(from: wetURL, range: range) }

        var accumulator = StreamingAccumulator(hopFrameCount: hopFrameCount)
        let timing = try await AudioTrackReader.readMix(
            sources: [
                .init(url: dryURL, gain: Float(1 - strength)),
                .init(url: wetURL, gain: Float(strength)),
            ],
            outputSettings: pcmOutputSettings(),
            range: range
        ) { buffer in
    ...
    private static func validate(range: ClosedRange<Double>?) throws {
        if let range {
    ...
    private static func pcmOutputSettings() -> [String: Any] {
        [
    ...
    private static func finish(
        accumulator: inout StreamingAccumulator,
        range: ClosedRange<Double>?,
        firstPresentationSeconds: Double?
        ...
        let requestedStart = range?.lowerBound ?? 0
        let sourceStart = max(requestedStart, firstPresentationSeconds ?? requestedStart)
