...
enum SocialAudioDucking {
    ...
    static func timelineSpeechRanges(
        ...
        // Match Clip.sourceFramesConsumed so fractional speeds end on the same
        // source-frame boundary used by composition insertion.
        let consumedSourceFrames = Double(Int((Double(clipDurationFrames) * speed).rounded()))
