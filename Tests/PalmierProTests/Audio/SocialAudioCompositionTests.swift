...
struct SocialAudioCompositionTests {
    ...
    @Test func normalizationMultipliesBothDenoiseBranchesWithoutChangingTheirBlend() throws {
        var voice = Fixtures.clip(
            id: "voice",
            mediaRef: "v",
            mediaType: .audio,
            start: 0,
        ...
        voice.effects = [Effect(
            ...
            params: ["amount": EffectParam(value: 0.6)]
        )]
        voice.socialAudio = SocialAudioSettings(
            role: .voice,
            normalizationGainDb: 6.020599913279624
        ...
        let timeline = Fixtures.timeline(
            fps: 30,
            tracks: [Fixtures.audioTrack(clips: [voice])]
        )

        let composition = AVMutableComposition()
        let dryTrack = try #require(composition.addMutableTrack(
            withMediaType: .audio,
            preferredTrackID: kCMPersistentTrackID_Invalid
        ))
        let wetTrack = try #require(composition.addMutableTrack(
            withMediaType: .audio,
            preferredTrackID: kCMPersistentTrackID_Invalid
        ))
        let mix = CompositionBuilder.buildVisuals(
            ...
            trackMappings: [
                mapping(
                    ...
                    trackIndex: 0,
                    clipId: voice.id,
                    blendedClipIds: [voice.id]
                ),
                mapping(track: wetTrack, trackIndex: 0, clipId: voice.id, wetAudio: true),
            ],
            compositionDuration: CMTime(value: 60, timescale: 30),
            renderSize: CGSize(width: 1080, height: 1920)
        ).audioMix

        let dryParams = try #require(mix.inputParameters.first { $0.trackID == dryTrack.trackID })
        let wetParams = try #require(mix.inputParameters.first { $0.trackID == wetTrack.trackID })
        let dry = try #require(volumeRamp(dryParams, atFrame: 30, fps: 30))
        let wet = try #require(volumeRamp(wetParams, atFrame: 30, fps: 30))
        #expect(abs(dry.start - 0.4) < 0.000_1)
        #expect(abs(dry.end - 0.4) < 0.000_1)
        #expect(abs(wet.start - 0.6) < 0.000_1)
        #expect(abs(wet.end - 0.6) < 0.000_1)
    }
